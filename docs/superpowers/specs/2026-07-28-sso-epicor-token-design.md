...
- Formato del JWT observado: header `{"alg":"HS256","typ":"JWT"}`, payload `{"exp":"<epoch>","iat":"<epoch>","iss":"epicor","aud":"<variable>","username":"<usuario>"}`. **`exp`/`iat` van codificados como strings, no como números** — cualquier token que este diseño emita debe replicar ese formato exacto para que Epicor lo acepte.
- **`aud` no tiene un valor fijo — confirmado en producción, 2026-07-29.** Un login interactivo fresco en Kinetic produce `aud: "epicor"`, pero cuando Kinetic renueva el token en segundo plano (silenciosamente, para mantener la sesión nativa viva más allá de la hora de vida del token) produce `aud: "00000000-0000-0000-0000-000000000000"`. Ambos son aceptados por la propia REST API de Epicor como Bearer válido (confirmado por Swagger) — la validación de este diseño solo exige `iss == "epicor"`, nunca compara `aud` contra un valor fijo.
...
- `EpicorTokenServiceTests`: valida un token bien firmado (construido de forma independiente al servicio, nunca contra su propio encoder); rechaza firma inválida (Sign Key distinta); rechaza `exp` pasado; rechaza `iss` distinto de `"epicor"`; **acepta cualquier valor de `aud`** (incluyendo el GUID de ceros que usa Kinetic al renovar en segundo plano); rechaza cuando la Sign Key configurada está vacía; expone correctamente el `expiresAtUtc` del token válido.
